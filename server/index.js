...
const buyTable = require('./modals/buyModels')
        ...
        .then(productUser => {
            if (productUser) {
                ...
                cartTable.create({
                    ...
                    .then(data => res.send({ msg: "a",d:data }))
...
app.post("/api/JWTToken", async (req, res) => {
    ...
    userTable.findOne({ _id: id })
        .then(data => res.send({ name: data.name, email: data.email, id: id }))
        .catch(err => res.json(err))
})
...
app.post("/api/buyNow", async (req, res) => {
    const { productId, userEmail, price, totalPrice, disprice, tax, quantity } = req.body;
    buyTable.create({
        productId: productId,
        userEmail: userEmail,
        price: price,
...
const userDataRoute = require('./Route/userroutes');
