...
export const ProductPage = ({id}) => {
  ...
  const beat = products.filter(headphone => headphone.name.includes('boAt')).slice(firstPageIndex,lastPageIndex);
  ...
  return (
    <div className='ProductPage'>
      ...
      <div className="col-md-11 mx-auto">
        ...
        {
          beat.map(prd=> {
            return(<ProductCard key={prd.id} col_size={col_size} img={prd.image} name={prd.name} price={prd.actual_price} rating={prd.ratings} id={prd.id} userid = {id}/>)
          })
        }
      </div>
      </div>
